<template>
  <div class="flex flex-col min-h-fit p-5 pb-8 text-gray-800">
      <!-- Component Start -->
	<h1 class="text-lg ml-4 mt-3 uppercase font-bold">os melhores filmes de {{ new Date().getFullYear() }}</h1>
	<div class="flex flex-col sm:flex-row sm:items-end sm:justify-between mt-6">
		<span class="text-sm font-semibold ml-4 text-justify">Seleção especial</span>
	</div>
...
export default {
    ...
    async mounted(){
        try{
           await this.$http.get('/discover/movie?primary_release_year='+new Date().getFullYear()).then(response=>{
